  ...
  try {
    ...
    const users = await User.find({
      ...
      $or: [
        ...
        { skillYouHave: { $regex: search, $options: "i" } },
        { skillYouWant: { $regex: search, $options: "i" } },
      ],
    })
      .select("-password")
      .limit(20);
...
export const getFriendsRequest = async (req, res) => {
  try {
    const incomingRequest = await friendRequest
      .find({
    ...
    const acceptedRequest = await friendRequest
      .find({
        sender: req.user.id,
        status: "accepted",
      })
...
export const sendFriendsRequest = async (req, res) => {
  try {
    ...
    if (recipient.friends.includes(myId)) {
      return res.status(400).json({
        message: "You were already Friends Of This Users",
      });
    }
    ...
    if (existingRequest) {
      return res.status(400).json({
        message: "friend request were already exists between you and this user",
      });
    }
